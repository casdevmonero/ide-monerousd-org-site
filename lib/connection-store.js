...
async function tryGetAccountSilent() {
  ...
  try {
    // Prefer getAddressSilent (no popup) but fall back to getAddress.
    // Both may return either a bare string OR an object
    // `{ address, subaddress_index }` — normalise to the address string
    // before mapping into the store. Pre-v1.2.215 the silent path only
    // accepted strings, silently dropping the object-shape response
    // from current wallets and leaving the pill stuck on "Connect".
    const fn = provider.getAddressSilent ?? provider.getAddress;
    const result = await fn.call(provider);
    const addr = (typeof result === 'string') ? result : (result?.address ?? null);
    if (addr) {
      set('connection', {
        status: 'connected',
...
export async function connect() {
  ...
  connectInflight = (async () => {
    try {
      // Two-step connect, mirroring `ion-monerousd-org/frontend/app.js`:
      //   1. provider.connect() authorizes the origin and returns the
      //      per-origin subaddress (rec.subaddress) on first connect,
      //      OR the cached value on reconnect.
      //   2. provider.getAddress() is the canonical "what's my address
      //      right now?" call. Calling it explicitly post-connect
      //      defends against drift: if connect() ever returned a stale
      //      cached value (e.g. the user's main address from a
      //      pre-per-origin-subaddress wallet build, or a value from a
      //      previous wallet identity), getAddress() always returns
      //      the wallet's current authoritative per-origin subaddress.
      //      Ionswap pattern; v1.2.215 IDE adopts it.
      const connectResult = await provider.connect();
      let addressObj = null;
      try {
        addressObj = await provider.getAddress();
      } catch (_) {
        // Wallet doesn't expose getAddress (very old build) → fall
        // back to whatever connect() returned. We never want to fail
        // the connect just because the secondary fetch failed.
        addressObj = connectResult;
      }
